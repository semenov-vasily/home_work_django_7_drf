# Домашнее задание "Django REST framework: фильтрация, валидация и аутентификация"

# Backend для приложения с объявлениями

## Порядок работы

Запросы подготовлены в файле [requests-examples.http](requests-examples.http)

1. заполнять БД, редактировать записи,привязывать объявления к пользователям и т.п. можно из админ-панели
2. Пагинация выполнена 5 записей на страницу
3. Выполнена валидация на количество записей со татусом OPEN для каждого пользователя перед созданием нового объявления или изменения статуса на OPEN у записанного в БД.
